...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(1, 28, 28), weight_scale=1, reg=0.0, dtype=cp.float32):
        self.params = {}
        self.reg = reg
        self.dtype = dtype
        ...
        C, H, W = input_dim
        # 32 * 32
        self.params['W1_1'] = weight_scale * cp.random.normal(loc=0, scale=(2 / (3 * 28 * 28)) ** 0.5,
                                                              size=(64, 3, 3, 3))
        self.params['W1_2'] = weight_scale * cp.random.normal(loc=0, scale=(2 / (64 * 28 * 28)) ** 0.5,
                                                              size=(64, 64, 3, 3))  # (N, D)
        self.params['b1_1'] = cp.zeros(64)
        self.params['b1_2'] = cp.zeros(64)
        self.params['beta1_1'] = cp.zeros(50176)  # 存在独立的维度参数  (D,1)  # 28*28*64
        self.params['gamma1_1'] = cp.ones(50176)
        self.params['beta1_2'] = cp.zeros(50176)
        self.params['gamma1_2'] = cp.ones(50176)

        # 16 * 16
        self.params['W2_1'] = weight_scale * cp.random.normal(loc=0, scale=(2 / (64 * 14 * 14)) ** 0.5,
                                                              size=(128, 64, 3, 3))
        self.params['W2_2'] = weight_scale * cp.random.normal(loc=0, scale=(2 / (128 * 14 * 14)) ** 0.5,
                                                              size=(128, 128, 3, 3))
        self.params['b2_1'] = cp.zeros(128)
        self.params['b2_2'] = cp.zeros(128)
        self.params['beta2_1'] = cp.zeros(14*14*128)  # 存在独立的维度参数  (D,1)  # 14*14*128
        self.params['gamma2_1'] = cp.ones(14*14*128)
        self.params['beta2_2'] = cp.zeros(14*14*128)
        self.params['gamma2_2'] = cp.ones(14*14*128)

        # 8 * 8
        self.params['W3_1'] = weight_scale * cp.random.normal(loc=0, scale=(2 / (128 * 7 * 7)) ** 0.5,
                                                              size=(256, 128, 3, 3))
        self.params['W3_2'] = weight_scale * cp.random.normal(loc=0, scale=(2 / (256 * 7 * 7)) ** 0.5,
                                                              size=(256, 256, 3, 3))
        ...
        self.params['beta3_1'] = cp.zeros(7*7*256)  # 存在独立的维度参数  (D,1)  # 7*7*256
        self.params['gamma3_1'] = cp.ones(7*7*256)
        self.params['beta3_2'] = cp.zeros(7*7*256)
        self.params['gamma3_2'] = cp.ones(7*7*256)

        # 4 * 4
        self.params['W4_1'] = weight_scale * cp.random.normal(loc=0.0, scale=(2 / (256 * 3 * 3)) ** 0.5,
                                                              size=(512, 256, 3, 3))
        self.params['W4_2'] = weight_scale * cp.random.normal(loc=0.0, scale=(2 / (512 * 3 * 3)) ** 0.5,
                                                              size=(512, 512, 3, 3))
        self.params['b4_1'] = cp.zeros(512)
        self.params['b4_2'] = cp.zeros(512)
        self.params['beta4_1'] = cp.zeros(4608)  # 存在独立的维度参数  (D,1)  # 3*3*512
        self.params['gamma4_1'] = cp.ones(4608)
        self.params['beta4_2'] = cp.zeros(4608)
        self.params['gamma4_2'] = cp.ones(4608)

        # # 2 * 2
        # self.params['W5_1'] = weight_scale * cp.random.normal(loc=0.0, scale=(2 / (512 * 2 * 2)) ** 0.5,
        #                                                       size=(512, 512, 3, 3))
        # self.params['W5_2'] = weight_scale * cp.random.normal(loc=0.0, scale=(2 / (512 * 2 * 2)) ** 0.5,
        #                                                       size=(512, 512, 3, 3))
        ...
        # self.params['beta5_2'] = cp.zeros(2048)
        # self.params['gamma5_2'] = cp.ones(2048)
        ...
        self.params['W6'] = weight_scale * cp.random.normal(loc=0, scale=(2 / (512 * 3 * 3)) ** 0.5,
                                                            size=(3 * 3 * 512, 10))
    ...
    def loss(self, X, y=None):
        ...
        # W6_1, b6_1 = self.params['W6_1'], self.params['b6_1']
        ...
        #         self.params['beta_' + str(i + 1)] = np.zeros(hidden_dims[i])
        #         self.params['gamma_' + str(i + 1)] = np.ones(hidden_dims[i])
        ...
        # bn_param5_2 = {'mode': 'train', 'eps': 1e-5, 'gamma': self.params['gamma5_2'], 'beta': self.params['beta5_2']}
        # bn_param6_1 = {'mode': 'train', 'eps': 1e-5, 'gamma': self.params['gamma6_1'], 'beta': self.params['beta6_1']}

        # test mode
        # bn_param1_1 = {'mode': 'test', 'eps': 1e-5, 'gamma': self.params['gamma1_1'], 'beta': self.params['beta1_1']}
        # bn_param1_2 = {'mode': 'test', 'eps': 1e-5, 'gamma': self.params['gamma1_2'], 'beta': self.params['beta1_2']}
        # bn_param2_1 = {'mode': 'test', 'eps': 1e-5, 'gamma': self.params['gamma2_1'], 'beta': self.params['beta2_1']}
        # bn_param2_2 = {'mode': 'test', 'eps': 1e-5, 'gamma': self.params['gamma2_2'], 'beta': self.params['beta2_2']}
        # bn_param3_1 = {'mode': 'test', 'eps': 1e-5, 'gamma': self.params['gamma3_1'], 'beta': self.params['beta3_1']}
        # bn_param3_2 = {'mode': 'test', 'eps': 1e-5, 'gamma': self.params['gamma3_2'], 'beta': self.params['beta3_2']}
        # bn_param4_1 = {'mode': 'test', 'eps': 1e-5, 'gamma': self.params['gamma4_1'], 'beta': self.params['beta4_1']}
        # bn_param4_2 = {'mode': 'test', 'eps': 1e-5, 'gamma': self.params['gamma4_2'], 'beta': self.params['beta4_2']}
        ...
        feature_2_1, cache_2_1 = conv_bn_relu_forward(feature_1_3, W2_1, b2_1, conv_param, bn_param2_1)
        ...
        # feature_4_3, cache_4_3 = max_pool_forward(feature_4_2, pool_param)
        
        # feature_5_3 = dropout_forward(feature_4_3, 0.5)
        # feature_5_1, cache_5_1 = conv_bn_relu_forward(feature_4_3, W5_1, b5_1, conv_param, bn_param5_1)
        # feature_5_2, cache_5_2 = conv_bn_relu_forward(feature_5_1, W5_2, b5_2, conv_param, bn_param5_2)
        # feature_5_3, cache_5_3 = max_pool_forward(feature_5_2, pool_param)

        # feature_6_1, cache_6_1 = conv_bn_relu_forward(feature_5_3, W6_1, b6_1, conv_param, bn_param6_1)
        if y is not None:
            feature_4_2 = dropout_forward(feature_4_2, 0.5)
        # print(feature_4_2.shape)
        ...
        # dfeature_6_1, dW6_1, db6_1, dgamma6_1, dbeta6_1 = conv_bn_relu_backward(dfeature_6, cache_6_1)

        # dfeature_5_3 = max_pool_backward(dfeature_6, cache_5_3)
        # dfeature_5_2, dW5_2, db5_2, dgamma5_2, dbeta5_2 = conv_bn_relu_backward(dfeature_6, cache_5_2)
        # dfeature_5_1, dW5_1, db5_1, dgamma5_1, dbeta5_1 = conv_bn_relu_backward(dfeature_5_2, cache_5_1)

        # dfeature_4_3 = max_pool_backward(dfeature_6, cache_4_3)
        ...
        grads = {'W1_1': dW1_1, 'b1_1': db1_1, 'W1_2': dW1_2, 'b1_2': db1_2,
                ...
                #  'W5_1': dW5_1, 'b5_1': db5_1, 'W5_2': dW5_2, 'b5_2': db5_2,
                #  'gamma5_1': dgamma5_1, 'beta5_1': dbeta5_1, 'gamma5_2': dgamma5_2, 'beta5_2': dbeta5_2,
                 # 'W6_1': dW6_1, 'b6_1': db6_1,
                 # 'gamma6_1': dgamma6_1, 'beta6_1': dbeta6_1,
                 'W6': dW6, 'b6': db6
                 }
